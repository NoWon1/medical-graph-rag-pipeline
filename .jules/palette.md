...
## 2024-05-24 - Retain Focus Visibility in Streamlit Apps
**Learning:** Default Streamlit custom CSS implementations often aggressively strip `outline` and `box-shadow` properties, which significantly harms keyboard accessibility.
**Action:** Always ensure explicit `:focus-visible` and `:focus` states are retained for interactive elements like buttons and text areas using brand-consistent colors.
