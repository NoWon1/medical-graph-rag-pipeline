...
def _cosine(v1: np.ndarray, v2: np.ndarray) -> float:
    # ⚡ Bolt: Vectorized cosine similarity using NumPy for significant speedup over pure Python loops.
    # Result is explicitly cast to native float() to prevent downstream JSON serialization errors.
...
def mmr_rerank(query: str, candidates: List[Document], embed_model: HuggingFaceEmbeddings, k: int = K_MMR_FINAL, lambda_mult: float = MMR_LAMBDA) -> List[Document]:
    ...
    # ⚡ Bolt: Pre-cast embeddings to numpy arrays outside the hot loop to prevent redundant conversion overhead
