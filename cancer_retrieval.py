...
def _cosine(v1: Union[List[float], np.ndarray], v2: Union[List[float], np.ndarray]) -> float:
    # ⚡ Bolt: Vectorized cosine similarity calculation using NumPy for performance
...
def mmr_rerank(query: str, candidates: List[Document], embed_model: HuggingFaceEmbeddings, k: int = K_MMR_FINAL, lambda_mult: float = MMR_LAMBDA) -> List[Document]:
    ...
    # ⚡ Bolt: Pre-cast to NumPy arrays outside hot loop for performance
    query_vec_arr = np.asarray(query_vec)
    doc_vecs_arr = [np.asarray(v) for v in doc_vecs]
    relevance = [_cosine(v, query_vec_arr) for v in doc_vecs_arr]
    ...
    for _ in range(min(k, len(candidates))):
        if not selected: best = max(remaining, key=lambda i: relevance[i])
            ...
            for idx in remaining:
                max_sim = max(_cosine(doc_vecs_arr[idx], doc_vecs_arr[s]) for s in selected)
