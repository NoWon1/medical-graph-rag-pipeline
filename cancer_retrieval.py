...
# ⚡ Bolt: Vectorized cosine similarity using NumPy for significant performance gains over pure Python loops.
# Explictly casts to float() to prevent JSON serialization errors from np.float64 types.
def _cosine(v1: Union[List[float], np.ndarray], v2: Union[List[float], np.ndarray]) -> float:
    v1_arr = v1 if isinstance(v1, np.ndarray) else np.array(v1)
    v2_arr = v2 if isinstance(v2, np.ndarray) else np.array(v2)
...
def mmr_rerank(query: str, candidates: List[Document], embed_model: HuggingFaceEmbeddings, k: int = K_MMR_FINAL, lambda_mult: float = MMR_LAMBDA) -> List[Document]:
    ...
    # ⚡ Bolt: Pre-cast vectors to NumPy arrays to avoid implicit conversion overhead during nested MMR iterations.
...
def _retrieve_image_chunks(query: str) -> List[Document]:
    ...
    try:
        ...
        # ⚡ Bolt: Pre-cast vectors to NumPy arrays to avoid implicit conversion overhead during semantic similarity calculation loop.
        query_vec = np.array(embed_model.embed_query(query))
