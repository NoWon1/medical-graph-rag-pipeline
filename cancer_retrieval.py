...
def _cosine(v1: Union[List[float], np.ndarray], v2: Union[List[float], np.ndarray]) -> float:
    # ⚡ Bolt: Vectorized cosine similarity using NumPy for performance.
    # Casting the result to a native Python float to prevent JSON serialization errors with np.float64.
...
def mmr_rerank(query: str, candidates: List[Document], embed_model: HuggingFaceEmbeddings, k: int = K_MMR_FINAL, lambda_mult: float = MMR_LAMBDA) -> List[Document]:
    ...
    # ⚡ Bolt: Pre-cast query and document embeddings to NumPy arrays to avoid implicit conversion overhead inside loops.
...
def _retrieve_image_chunks(query: str) -> List[Document]:
    ...
    try:
        ...
        # ⚡ Bolt: Pre-cast query and document embeddings to NumPy arrays to avoid implicit conversion overhead in loops.
        query_vec = np.array(embed_model.embed_query(query))
