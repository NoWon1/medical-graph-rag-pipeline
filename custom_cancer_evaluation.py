# =============================================================================
# cancer_evaluation.py  v4.0  — LLM-Free Custom Metrics Edition
#
# DESIGN PHILOSOPHY:
...
#
# HOW TO RUN (no RAGAS needed, no token cost):
...
EVAL_DIR      = Path(__file__).parent / "output" / "custom_evaluation"
...
# =============================================================================
# CONTEXT RETRIEVAL (mirrors what cancer_retrieval.py does internally)
# =============================================================================

def _get_contexts(question: str, query_mode: str) -> list[str]:
    """Retrieve contexts the pipeline would use for this question."""
    try:
        fn = {
...
def run_evaluation(
    ...
    """
    Run all 7 custom metrics on the test set.
    Zero LLM calls for scoring — only the answer generation uses Groq.

    Returns full results dict with per-question scores and aggregates.
    """
    print("=" * 70)
    ...
    for i, item in enumerate(items, 1):
        ...
        try:
            # Generate answer through the real pipeline
            t0 = time.time()
            ...
            # Retrieve contexts (to measure utilisation)
            contexts = _get_contexts(q, mode)
