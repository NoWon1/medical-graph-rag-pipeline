...
def _cosine(v1, v2) -> float:
    # ⚡ Bolt: Vectorize cosine similarity calculation with NumPy for faster execution
    # Accepts numpy arrays to prevent casting overhead in hot loops
...
def mmr_rerank(query: str, candidates: List[Document], embed_model: HuggingFaceEmbeddings, k: int = K_MMR_FINAL, lambda_mult: float = MMR_LAMBDA) -> List[Document]:
    ...
    # ⚡ Bolt: Cast query and document vectors to numpy arrays once before the MMR loop
    # to avoid repeated conversion overhead in _cosine
...
def _retrieve_image_chunks(query: str) -> List[Document]:
    ...
    try:
        ...
        # ⚡ Bolt: Pre-cast query and document vectors for faster cosine computation
        query_vec = np.array(embed_model.embed_query(query))
