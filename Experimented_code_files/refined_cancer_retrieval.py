...
#     """
#     FIX 1 (Accuracy) & FIX 2 (Deduplication)
#     Uses BM25 to cast a wide net, deduplicates based on the explicit image filename, 
#     and then uses the local BAAI embedding model to rerank images based on meaning.
#     """
...
#         # 2. FIX 2: Deduplicate images using regex
#         seen_filenames = set()
#         unique_candidates = []
...
#         filtered = []
#         for sim, doc in scored_candidates:
#             # High threshold or explicit visual intent
#             if has_visual_intent or sim > 0.50:
#                 filtered.append(doc)
        
#         img_retriever.k = 3 # Reset
#         return filtered[:2]
...
# def _build_sources(vector_docs: List[Document], graph_intent: str = "", image_docs: List[Document] = None) -> list[dict]:
#     sources, seen = [], set()
...
#         sf = doc.metadata.get("source_file", "").strip() or re.sub(r'_cap_\d+$|_\d{4}$', '', doc.metadata.get("chunk_id", ""))
...
def generate_answer(query: str, patient_report: str = "", chat_history: list = None, cancer_filter: str = "", query_mode: str = QUERY_MODE_DEFAULT) -> tuple[str, list]:
    ...
    try:
        ...
        #     return _web_search_fallback(answer, query, patient_report, rag_is_empty=(len(answer.strip()) < 300))
        # return answer, src
...
def generate_answer_stream(query: str, patient_report: str = "", chat_history: list = None, cancer_filter: str = "", query_mode: str = QUERY_MODE_DEFAULT):
    ...
    try:
        ...
        #     for char in fallback_ans[len(full_answer):]: yield char
        #     full_answer, src = fallback_ans, fallback_src
