...
# ⚡ BOLT OPTIMIZATION: Replaced pure Python math loop with vectorized NumPy
# dot product for O(n) similarity calculations. This prevents computational
# bottlenecks in MMR reranking and image semantic reranking loops.
...
def mmr_rerank(query: str, candidates: List[Document], embed_model: HuggingFaceEmbeddings, k: int = K_MMR_FINAL, lambda_mult: float = MMR_LAMBDA) -> List[Document]:
    ...
    # Pre-cast to NumPy arrays before hot loops
...
def _retrieve_image_chunks(query: str) -> List[Document]:
    ...
    try:
        ...
        # Pre-cast to NumPy arrays for vectorized calculations
        query_vec_np = np.array(query_vec)
        doc_vecs_np = np.array(doc_vecs)
