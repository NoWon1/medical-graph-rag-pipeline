...
## 2024-05-30 - Prevent Information Leakage in API/Streamlit Handlers
**Vulnerability:** Unhandled exceptions yielding raw `str(e)` and using `traceback.print_exc()` directly in generator/retrieval functions, which leaked internal stack traces and implementation details to the Streamlit UI.
**Learning:** Returning exception messages up the call stack to a UI component exposes sensitive server-side details to the client.
**Prevention:** Always log exceptions safely to the server console using `logging.error(..., exc_info=True)` and yield/return generic, secure fallback messages to the user.
